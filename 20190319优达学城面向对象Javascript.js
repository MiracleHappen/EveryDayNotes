...
而且还可以实现许多强大的功能，如构造函数。这是一个面向对象的 JavaScript 课程，因此我们将在本课程中深入探索这些功能！
...
现在，让我们比较一下它和 whoThis() 函数。由于它作为一个常规函数被调用（而不是作为对象上的方法被调用），因此它的调用如下所示：

whoThis();
如你所见，里面没有点。因此，点的左侧也没有任何对象。那么，whoThis() 函数中的 this 的值是什么呢？这是 JavaScript 语言一个有趣的特点。
